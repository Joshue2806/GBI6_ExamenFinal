...
```{r funcion_long}
long_df <- function(df) {
  df$dataset <- as.factor(df$bcr_patient_barcode)
  df$dataset <- as.factor(df$dataset)
  library("tidyr")
  long <- gather(df, gen, expresion_level, -c(bcr_patient_barcode, dataset))
  }
...
library(sjmisc)
library(dplyr)  
agrupt <- df_long %>% group_by(dataset,gen)
agrupt <- descr(x=BRCA, out="browser",
...
#de los datos estadísticos de manera agrupada.
#Tras analizar los valores obtenidos se puede determinar que los cánceres de 
#
